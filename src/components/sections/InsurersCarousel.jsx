...
const INSURERS = [
  ...
  // Logos à ajouter quand disponibles
  { name: 'Malakoff Humanis', logo: null },
  { name: 'CNP Assurances',   logo: null },
  { name: 'BNP Cardif',       logo: null },
  { name: 'Prévoir',          logo: null },
]
...
const LogoCard = ({ name, logo }) => {
  ...
  return (
    <div className="flex-shrink-0 mx-4 bg-white rounded-2xl px-6 py-4 shadow-sm border border-slate-100 flex items-center justify-center gap-3 min-w-[160px] hover:shadow-md transition-shadow">
      {logo && !error ? (
        <img
          src={logo}
          alt={`Logo ${name}`}
