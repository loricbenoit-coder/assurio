...
const STEPS = [
  {
    ...
    description: "Répondez à quelques questions simples sur votre prêt et votre situation personnelle. Ça prend moins de 2 minutes.",
  },
  {
    icon: Search,
    number: '02',
    title: 'Comparez les offres',
    description: "Nous comparons pour vous les meilleures assurances emprunteur du marché en temps réel.",
  },
  {
    icon: FileCheck,
    number: '03',
    title: 'Souscrivez en ligne',
    description: "Choisissez votre offre et signez électroniquement. Nous gérons les formalités avec votre banque.",
  },
  {
    icon: Smile,
    number: '04',
    title: 'Économisez tout de suite',
    description: "Votre nouvelle assurance prend effet immédiatement. Vous économisez dès le premier mois.",
  },
]
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 relative">
        ...
        {STEPS.map(({ icon: Icon, number, title, description }) => (
          <div key={number} className="relative flex flex-col items-center text-center group">
            <div className="relative z-10 w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-[#f0f4ff] border-2 border-white shadow-lg flex items-center justify-center mb-4 md:mb-6 group-hover:bg-[#0f1f6b] transition-colors duration-300">
              <Icon className="w-6 h-6 md:w-7 md:h-7 text-[#0f1f6b] group-hover:text-white transition-colors duration-300" />
            ...
            <h3 className="text-sm md:text-lg font-semibold text-[#0a1340] mb-1 md:mb-2">{title}</h3>
