        ...
        <div className="flex items-center gap-1.5">
          <div className={cn(
            ...
            i < current ? 'bg-[#10b981] text-white'
              : i === current ? 'bg-[#0f1f6b] text-white'
...
const QuoteCard = ({ quote, rank, selected, onSelect }) => (
  <div
    className={cn(
      ...
      selected ? 'border-[#0f1f6b] shadow-lg shadow-[#0f1f6b]/10 ring-1 ring-[#0f1f6b]'
        : rank === 0 ? 'border-[#10b981] shadow-md shadow-emerald-100 hover:shadow-lg'
      ...
      const res = await fetch('/.netlify/functions/send-lead', {
        ...
        body: JSON.stringify({
          ...
          loanInfo: {
            ...
            age: form.age, smoker: form.smoker, profession: form.profession,
          },
                  ...
                  {[10, 15, 20, 25].map((y) => (
                    <button key={y} onClick={() => set('duration', y)}
                      className={cn('flex-1 py-2 text-sm rounded-xl border font-medium transition-all',
                        Number(form.duration) === y ? 'bg-[#0f1f6b] text-white border-[#0f1f6b]'
